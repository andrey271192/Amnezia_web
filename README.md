...
В старых версиях под **`set -o pipefail`** подсчёт контейнеров через **`grep`** падал, если **нет ни одного запущенного имени `amnezia-awg…`** (например WG остановлен или имя другое): скрипт завершался до строки **«Сборка образа»**. Обновите **`scripts/install.sh`** до актуального из `main` или не используйте локальную копию без исправления.
